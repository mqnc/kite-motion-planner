...
Another way is to limit the amount of simultaneously-actuated joints. RRTConnect can still work quite well if it can only move one or two joints at a time.
...
An advantage is that it can potentially speed up exploration, as we can add every corner of a free cuboid to the tree/map of valid states each check.
